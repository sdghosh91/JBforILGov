...
# code for extracting zip file -------------------------------------------------
    ...
    for i in os.listdir(path): # looks in path folder
        ...
        if os.path.isfile(os.path.join(path,i)) and i.endswith('.zip') and i.startswith(fileName1):
    ...
    for k in os.listdir(path):
        if os.path.isfile(os.path.join(path,k)) and k.endswith('.txt') and k.startswith(fileName1):
        ...
        if os.path.isfile(os.path.join(path,k)) and k.endswith('.xls') and k.startswith(fileName3):
            n = os.path.join(path,k) # this is the extracted file!
# ------------------------------------------------------------------------------

# import txt file into a tempList : duplicate code for each new file -----------
    tempList1 = []  # test should be initialized in the function body
    with open(os.path.abspath(m), 'rt') as f:
        reader = csv.reader(f, delimiter =  '\t')
        for row in reader:
            tempList1.append(row)
        df = 'tempList1' # call time 6PM
...
# add data from tempList : duplicate code for each new file --------------
    ...
    result = service.spreadsheets().values().append(
        spreadsheetId=spreadsheetId1, range=rangeName1,
